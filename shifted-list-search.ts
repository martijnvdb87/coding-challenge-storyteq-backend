const getLargestInteger = (input: number[]): number | null => {
  if (input.length < 1) {
    return null;
  }
  ...
  const check = (input: number[]): number[] => {
    // Length is 1 if the highest number is found
    if (input.length === 1) {
      return input;
    }

    const parts: [number[], number[]] = splitArray(input);
...
const splitArray = (input: number[]): [number[], number[]] => {
  const middle = Math.round(input.length / 2);

  const start = input.slice(0, middle);
  const end = input.slice(middle, input.length);
